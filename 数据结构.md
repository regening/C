...
哈希表是非常高效的数据结构，具有常数时间复杂度的查找、插入和删除操作。哈希表通过将数据映射到一个数组的位置（哈希值）来存储和查找数据。

**哈希表的基本概念：**

* **哈希函数**：将数据（通常是键）映射到数组的索引上。
* **冲突解决**：当两个键映射到相同的索引时，发生冲突。常见的冲突解决方法有：

  * **链地址法**：每个数组的元素是一个链表，当多个元素哈希到同一个位置时，它们就被链在一起。
  * **开放地址法**：当发生冲突时，寻找下一个空位置。
...
struct HashTable* create() {    // 创建一个新的哈希表
...
void insert(struct HashTable* ht, int key, int value) {     // 插入键值对
    int index = hash(key);
    struct Node* newNode = (struct Node*)malloc(sizeof(struct Node));
    newNode->key = key;
    newNode->value = value;
...
int search(struct HashTable* ht, int key) {     // 查找键值对
    int index = hash(key);
...
void print(struct HashTable* ht) {   // 打印哈希表
    for (int i = 0; i < TABLE_SIZE; i++) {
...
int main() {
    ...
    insert(ht, 1, 10);
    insert(ht, 2, 20);
    insert(ht, 12, 30);  // 12 % 10 == 2, 应该和key 2发生冲突
    insert(ht, 5, 50);
